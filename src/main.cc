...
void parseArgs(int argc, char* argv[], vector<string>& abilities, vector<vector<string>>& links, bool& graphicsEnabled, bool& extraGraphics) {
    const string ARG_ERROR_MSG = "Error, please use the argument options: -ability1 <order>, -ability2 <order>, -link1 <placement-file>, link2 <file> -graphics -extragraphics\n";
...
unique_ptr<GameBoard> parseCmds(istream& in, unique_ptr<GameBoard> gb,
                                ...
                                const bool& extraGraphics = false) {
    string cmd;
    while (in >> cmd) {
        if (gb->getIsWon()) {
        ...
        try {
            if (cmd == "move") {
                ...
                if(extraGraphics) gb->drawAbilities();
...
int main(int argc, char* argv[]) {
    ...
    bool extraGraphics = false;
    
    // cmd line args
    try {
        // set abilities, links, and graphicsEnabled and extraGraphics
        parseArgs(argc, argv, abilities, links, graphicsEnabled, extraGraphics);
    ...
    try {
        ...
        // text commands
        try {
            gb = parseCmds(cin, move(gb), false, extraGraphics);
